...
| `process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!` | Public | Browser Supabase authentication/data client | staging, production |
...
| `STRIPE_WEBHOOK_SECRET` | Server-only | Verifies signed Stripe webhook requests | staging test mode only |
| `STRIPE_PAYMENTS_ENABLED` | Server-only | Explicit test-mode opt-in for portal Checkout | staging test mode only; never production |
