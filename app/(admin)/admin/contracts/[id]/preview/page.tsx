...
export default async function ContractPreviewPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="mx-auto max-w-[8.5in] space-y-4 p-3">
    ...
    <article className="document-print relative overflow-hidden rounded-2xl bg-white p-6 shadow-xl">
      ...
      <section className="print-keep-together relative z-10 mt-6 rounded-2xl border border-slate-200 p-5"><h3 className="text-lg font-bold text-slate-800">Signature</h3><div className="mt-5 grid gap-6 sm:grid-cols-2"><div className="border-b border-slate-300 pb-2 text-sm">Customer: {contract.signed_by_name ?? "Pending signature"}</div><div className="border-b border-slate-300 pb-2 text-sm">Date: {contract.signed_at ? new Date(contract.signed_at).toLocaleDateString() : "Pending"}</div></div></section>
      <footer className="print-keep-together relative z-10 mt-8 border-t border-slate-200 pt-4 text-center text-xs text-slate-500">XAREON GROUP · Shield of Integrity · (202) 286-8497 · info@xareongroup.com</footer>
    </article>
  </div>;
}
