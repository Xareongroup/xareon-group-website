...
export default async function PortalContractPage({ params }: { params: Promise<{ token: string; id: string }> }) {
  const { token, id } = await params; const data = await getPortalContract(token, id); if (!data) notFound();
