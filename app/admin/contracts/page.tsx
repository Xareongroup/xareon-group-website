  ...
  async function loadContracts() {
    ...
    if (error) {
      ...
      (data ?? []).map((contract: any) => ({
        ...
        customer: Array.isArray(contract.customer)
          ? contract.customer[0] ?? null
          : contract.customer,

        estimate: Array.isArray(contract.estimate)
          ? contract.estimate[0] ?? null
          : contract.estimate,

        job: Array.isArray(contract.job)
          ? contract.job[0] ?? null
          : contract.job,
      }));
  ...
  return (
    ...
    <div className="mx-auto max-w-7xl px-6 py-8">
      ...
      <SearchBar
        ...
        placeholder="Search contract number, title or customer..."
        status={statusFilter}
